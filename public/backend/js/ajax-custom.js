...
async function lineChartCertificate() {
    var $primary = "#f43736";
    var $secondary = "#414091";
    var certificateCount, Monthdata;

    await $.ajax({
        url: "admin_certificate_dashboard_chart",
        type: "POST",
        data: {
            _token: $('meta[name="csrf-token"]').attr("content"),
        },
        ...
        success: function (result) {
            ...
            certificateCount = result.map(a => a.total);

            // certificateCount = Object.values(result.certificate_count).map(Number);
        },
        failure: function () {
