...
class LoginController extends Controller
{
    ...
    public function forgotPasswordStore(Request $request)
    {
        ...
        Mail::send('backend/auth/email-forgot', ['body' => $content], function ($message) use ($email) {
            $message->from('info@mypcot.com', 'Caltech Team');
            $message->to($email, 'Caltech Team')->subject('Forgot Password - Caltech Team');
        });
    ...
    public function passwordUpdate(Request $request)
    {
        ...
        $request->validate([
            ...
            'password' => ['required', 'confirmed', 'min:6'],         // changes by vikas
        ]);
